import type { SystemicIncident } from "@/lib/types";

export function SystemicMonitor({
  state,
  onTrigger,
  onReset,
  busy,
}: {
  state: SystemicIncident | null;
  onTrigger: () => void;
  onReset: () => void;
  busy: boolean;
}) {
  ...
  return (
    <div className={`systemic-bar ${active ? "systemic-alert" : ""}`}>
      <div className="systemic-left">
        ...
        <div>
          <div className="systemic-title">{active ? "SYSTEMIC PAYMENT DEGRADATION" : "Systemic Monitor — All systems normal"}</div>
          {active && <div className="systemic-sub">Automated recovery suppressed for affected cases</div>}
        </div>
      </div>
      ...
      <div className="systemic-stats">
        <div className="systemic-stat">
          ...
          <span className={`stat-value mono ${active ? "stat-danger" : ""}`}>{active ? "28%" : "3.2%"}</span>
        </div>
        <div className="systemic-stat">
