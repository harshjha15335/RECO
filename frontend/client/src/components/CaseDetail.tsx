...
import type { ExecuteResult, RecoveryCase } from "@/lib/types";
...
export function CaseDetail({
  ...
  onInjectFailure: (id: string, type: "gateway") => void;
  busy: boolean;
  ...
  const canExecute = status === "pending" && (chosen_action === "reminder_now" || chosen_action === "wait_and_payment_link");
      ...
      <div className="detail-header">
        <div>
          ...
          <div className="detail-meta mono">
            {caseData.invoice_number} · {formatRupees(caseData.invoice_amount)} · {caseData.days_overdue} days overdue
          </div>
        </div>
        <span className={`status-badge status-badge-${STATUS_COLOR[status] || "neutral"}`}>
      ...
      {diagnosis && (
        <>
              ...
              {options?.map((o) => (
                <div key={o.action} className={`option-row ${o.action === chosen_action ? "option-chosen" : ""}`}>
                  ...
                  <div className="option-bottom">
                    ...
                    {o.action !== "stop" && <span className="option-risk">Relationship risk: {o.relationship_risk}</span>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="detail-actions">
            <button className="primary-btn" onClick={() => onExecute(caseData.case_id)} disabled={!canExecute || busy}>
          ...
          {execResult && (
            <div className={`exec-result ${execResult.success ? "exec-success" : "exec-fail"}`}>
              {execResult.success ? `Success (${execResult.mode}) — ${execResult.link || execResult.message}` : `Failed — ${execResult.error}`}
            </div>
          )}
        </>
      )}
